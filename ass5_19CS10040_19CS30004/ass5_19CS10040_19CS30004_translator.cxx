...
sym* symtable::lookup(string name) 
{
	...
	/* If the symbol has not been found in the symbol table then craete a new entry for the symbol table and insert in the table & Return the pointer to this new element inserted */
    if(ST == this and !ptr){
        symbol = new sym(name);
...
void symtable::print()           
{
    int next_instr=0;
    list<symtable*> tb;           
    for(int t1=0;t1<65;t1++) std::cout<<"__";   
    std::cout<<std::endl;
    ...
    for(int x=0; x<65; x++) std::cout<<"__";  
    std::cout<<std::endl;
    ...
    {
		/* print symbol table that are nested in the current symbol table, hence recursively print all nested tables */
        (*it)->print();
    }
}
...
void quadArray::print()                                               
{
    ...
    for(int i=0;i<65;i++) std::cout<<"__";                        
    std::cout<<std::endl;
}
